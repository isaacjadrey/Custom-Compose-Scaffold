...
/**
 * State for [CustomScaffold] composable component
 *
 * Contains basic screen state, e.g. Drawer configuration, as well as sizes of components after
 * layout has happened
 *
 * @param customDrawerState the drawer state
 * @param snackbarHostState instance of [SnackbarHostState] to be used to show [Snackbar]s inside
 * of the [CustomScaffold]
 */
...
/**
 * Creates a [CustomScaffoldState] with the default animation clock and memorizes it.
 *
 * @param drawerState the custom drawer state
 * @param snackbarHostState instance of [SnackbarHostState] to be used to shoe [Snackbar]s inside
 * of the [CustomScaffold]
 */
@Composable
fun rememberCustomScaffoldState(
    drawerState: CustomDrawerState = rememberCustomDrawerState(initialValue = CustomDrawerValue.Closed),
    snackbarHostState: SnackbarHostState = remember { SnackbarHostState() },
): CustomScaffoldState {
    return remember { CustomScaffoldState(drawerState, snackbarHostState) }
}

/**
 * The possible positions for a [FloatingActionButton] attached to a [CustomScaffold]
 */
@JvmInline
 ...
 *
 * [Scaffold] is being customized to use the [CustomDrawer] instead of the [ModalDrawer]
 *
 * @param modifier optional Modifier for the root of the [Scaffold]
 * @param customScaffoldState state of this scaffold widget. It contains the state of the screen, e.g.
 ...
 * @param customDrawerContent content of the Drawer sheet that can be pulled from the left side (right
 * for RTL).
 ...
 * @param contentCornerSize size of the shape of the content.
...
fun CustomScaffold(
    ...
    customScaffoldState: CustomScaffoldState = rememberCustomScaffoldState(),
    ...
    customDrawerContent: @Composable (ColumnScope.() -> Unit)? = null,
    ...
    val child = @Composable { childModifier: Modifier ->
        Surface(modifier = childModifier, color = backgroundColor, contentColor = contentColor) {
            CustomScaffoldLayout(
                ...
                content = content,
                snackbar = {
                    snackbarHost(customScaffoldState.snackbarHostState)
                },
    ...
    if (customDrawerContent != null) {
        CustomDrawer(
            customDrawerContent = customDrawerContent,
            modifier = modifier,
            customDrawerState = customScaffoldState.customDrawerState,
...
/**
 * Layout for a [CustomScaffold]'s content.
 *
...
private fun CustomScaffoldLayout(
    ...
    SubcomposeLayout { constraints ->
        ...
        layout(layoutWidth, layoutHeight) {
            ...
            val customFabPlacement = if (fabPlaceables.isNotEmpty()) {
            ...
            val bottomBarPlaceables = subcompose(ScaffoldLayoutContent.BottomBar) {
                CompositionLocalProvider(
                    LocalCustomFabPlacement provides customFabPlacement,
                    content = bottomBar
                )
            }.fastMap { it.measure(looseConstraints) }

            val bottomBarHeight = bottomBarPlaceables.fastMaxBy { it.height }?.height ?: 0
            val fabOffsetFromBottom = customFabPlacement?.let {
            ...
            fabPlaceables.fastForEach {
                it.place(customFabPlacement?.left ?: 0, layoutHeight - (fabOffsetFromBottom ?: 0))
            }
        }
    }
}
 ...
 * location.
 */
internal val LocalCustomFabPlacement = staticCompositionLocalOf<FabPlacement?> { null }
